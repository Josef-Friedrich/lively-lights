...
class TestClassHue(unittest.TestCase):
    ...
    def setUp(self):
        if 'LIVELY_LIGHTS_IP' in os.environ:
            self.old_ip = os.environ['LIVELY_LIGHTS_IP']

        if 'LIVELY_LIGHTS_USERNAME' in os.environ:
            self.old_username = os.environ['LIVELY_LIGHTS_USERNAME']
    ...
    def tearDown(self):
        if hasattr(self, 'old_ip'):
            os.environ['LIVELY_LIGHTS_IP'] = self.old_ip

        if hasattr(self, 'old_username'):
            os.environ['LIVELY_LIGHTS_USERNAME'] = self.old_username
